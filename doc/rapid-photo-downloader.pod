...
=item B<-t PATH, --this-computer PATH>

The PATH on this computer from which to download, overwriting existing program 
preferences. Set PATH to '' to turn off.

=item B<--photo-destination PATH>

Sets the PATH where photos will be downloaded, overwriting existing program 
preferences.

=item B<--video-destination PATH>

Sets the PATH where videos will be downloaded, overwriting existing program 
preferences.
...
Turns on or off use of the Rapid Photo Downloader Thumbnail Cache. Does not delete existing
cache contents.
...
Resets all program settings to their default values, deletes all Thumbnail cache and which files
have been previously downloaded, and exits.
