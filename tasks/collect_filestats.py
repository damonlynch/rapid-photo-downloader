...
__description__ = 'Gather the file stats for all the files in a directory, and use them to apply ' \
...
def parser_options(formatter_class=argparse.HelpFormatter) -> argparse.ArgumentParser:
    ...
    group.add_argument(
        '-w', '--write', action='store', dest='stats',
        help='Write out the file stats to the directory of files using the stats file provided'
    )
    ...
    if args.collect:
        ...
        for dir_name, name in walk_file_system(path_to_walk):
            ...
            if stats:
                print(
                    '{:{}}{} {}'.format(
                        path, max_len + 2, datetime.fromtimestamp(stats.st_mtime).strftime(format),
