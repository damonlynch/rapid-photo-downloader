...
Do you want to proceed with the download?
    ...
    def cameraRemovedWhileThumbnailing(self, scan_id: int) -> None:
        ...
        try:
            ...
            logging.debug(
                "Got thumbnailing error from a camera that no longer exists (scan id %s)",
                scan_id,
            )
            return
    ...
    def cameraRemoved(self) -> None:
        ...
        for model, port in removed_cameras:
            ...
            if scan_id is None:
                logging.debug(
                    "The camera with scan id %s was already removed, or was never added",
                    scan_id,
                )
            else:
                device = self.devices[scan_id]
        ...
        if self.prefs.device_autodetection:
            ...
            if not self.prefs.scan_specific_folders or has_one_or_more_folders(
                ...
                logging.debug(
                    ...
                    "level that indicates it should be scanned",
                    path,
                )
        return False
